...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while distance <= self.max_range and not place.is_hive:
            if place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount < self.health:
            ...
            for bee in self.place.bees[:]:
                self.place.bees[self.place.bees.index(bee)].reduce_health(amount + self.damage)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if not gamestate.HasQueenAnt:
            gamestate.HasQueenAnt = True
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.slowed_turn > 0 and gamestate.time % 2 != 0:
            self.slowed_turn -= 1
        else:
            if self.slowed_turn > 0:
                self.slowed_turn -= 1
            destination = None
            if self.scared_turn > 0:
                self.scared_turn -= 1
