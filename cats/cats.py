...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def choose(setence):
        return any([lower(x) in topic for x in split(remove_punctuation(setence))])
    return choose
    # END PROBLEM 2
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        ...
        if current_diff <= limit and current_diff < least_diff:
            final_word = word
...
def final_diff(start, goal, limit):
    ...
    if lower(start[0]) == lower(goal[0]):  
        ...
        if len(goal) == 1:
            return len(start)
        if start[1] + start[0] == goal[0:2]:
            return final_diff(start[2:],goal[2:],limit - 1)
        if start[1] == start[0]:
            return final_diff(start[1:],goal,limit - 1)
        add = final_diff(goal[0] + start, goal, limit - 1)
        remove = final_diff(start[1:], goal, limit - 1)
        substitute = final_diff(start[1:], goal[1:], limit - 1)
        return 1 + min(add,remove,substitute)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    progress = 0
    for word in sofar:
        if prompt[progress] == word:
            progress += 1
        else:
            break
    pro = progress / len(prompt)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    def check_time(match, player_index, word_index):
        c_time = time(match, player_index, word_index)
        for i in player_indices:
            if time(match, i, word_index) < c_time:
                return False
            if time(match, i, word_index) == c_time and i < player_index:
                return False
        return True

    return [[word_at(match, word_index) for word_index in word_indices
        if check_time(match, player_index, word_index)] 
