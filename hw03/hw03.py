...
def count_coins(change):
    ...
    True
    """
    def count_coins_max(change, max):
        if change < 0:
            return 0
        if max == 1:
            return 1
        return count_coins_max(change - max, max) + count_coins_max(change, descending_coin(max))
    return count_coins_max(change, 25)
...
def make_anonymous_factorial():
    ...
    True
    """
    #to use it self, should transfer itself as a paramter 
    #def fact(f)(n):
    #if n == 1:
    #    return 1
    #else:
    #    return n * f(f)(n-1)
    #fact(fact)(9)
    return (lambda f: lambda n: f(f)(n))(lambda f: lambda n: 1 if n == 1 else n * f(f)(n - 1))
