...
class Coin:
    ...
    def worth(self):
        return self.cents + (lambda x : x if x > 0 else 0)(Mint.present_year - self.year - 50)
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value == 0:
            instance = VirFib(1)
            instance.previous_number = 0
        else:
            instance = VirFib(self.value + self.previous_number)
            instance.previous_number = self.value
        return instance
...
def is_bst(t):
    ...
    def bst_min(t):
        if t.is_leaf():
            ...
            for branch in t.branches:
                min_t = min(min_t, bst_min(branch))
            return min_t
    def bst_max(t):
        if t.is_leaf():
            return t.label
            ...
            max_t = t.label
            for branch in t.branches:
                max_t = max(max_t, bst_max(branch))
            return max_t
    if len(t.branches) > 2:
        return False
    if t.is_leaf():
    ...
    if len(t.branches) == 1:
        return (t.label >= bst_max(t.branches[0]) or t.label <= bst_min(t.branches[0]) ) and is_bst(t.branches[0])
