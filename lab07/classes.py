...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        for card in player.deck.cards:
            ...
            card.defense += opponent_card.defense
        i = 0
        while i < len(opponent.deck.cards) - 1:
            card = opponent.deck.cards[i]
            card_need_delete = False
            for other_card in opponent.deck.cards[i+1:]:
                if other_card.attack == card.attack or other_card.defense == card.defense:
                    opponent.deck.cards.remove(other_card)
                    card_need_delete = True
            if card_need_delete:
