def insert_into_all(item, nested_list):
    ...
    #the copy of list of list should be careful
    #must copy the list and the list that the items point to
    lists = []
    for list in nested_list:
        insert_list = list[:]
        insert_list.insert(0, item)
        #insert function return None
        lists.append(insert_list)
    return lists
...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        lists = subseqs(s[1:])
        result = insert_into_all(s[0], lists)
        result.extend(lists)
        return result
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    # num_trees(n) equal num_trees(left tree) * num_tree(rigth tree)
    if n == 1:
        return 1
    sum = 0
    i = 1
...
class Account:
    ...
    def withdraw(self, amount):
        ...
        self.transactions.append(('withdraw', 100))
        return self.balance
    ...
    def __repr__(self):
        return f"Accountholder: {self.holder}, Deposits: {self.deposit_time}, Withdraws: {self.withdraw_time}"
...
def insert(link, value, index):
    ...
    """
    if index == 0:
        reserve_value = link.first
        link.first = value
        link.rest = Link(reserve_value, link.rest)
        return
