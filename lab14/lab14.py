...
def prune_min(t):
    ...
    if t.is_leaf():
        ...
        t.branches = [min(t.branches, key=lambda branch: branch.label)]
...
class Player:
    ...
    def debate(self, other):
        if random() < max(0.1, self.popularity / (self.popularity + other.popularity)):
            ...
            self.popularity = (lambda x : 0 if x < 0 else x)(self.popularity - 50)

    def speech(self, other):
        self.votes += self.popularity // 10
        self.popularity = self.popularity + self.popularity // 10
        other.popularity = (lambda x : 0 if x < 0 else x)(other.popularity - other.popularity // 10)
...
def add_trees(t1, t2):
    ...
    if t2 is None:
        return t1
    branches = []
    for branch in zip_longest(t1.branches, t2.branches):
        branches.append(add_trees(*branch))
